...
void generarLaberinto(char** laberinto, int filas, int columnas, int jugadorX, int jugadorY, int salidaX, int salidaY) {
    ...
    bool** visitado = new bool*[filas];  // Este fragmento de código crea e inicializa una matriz 2D de booleanos de tamaño filas x columnas, donde cada celda se inicializa a false, indicando que ninguna celda ha sido visitada aún. Esta matriz es útil para rastrear las celdas que han sido visitadas durante la generación del laberinto o durante la búsqueda de una solución
    for (int i = 0; i < filas; ++i) {
    ...
    while (!cola.empty()) {  //Este bucle se usa para procesar cada celda de forma secuencial BFS
        ...
        for (int i = 0; i < 4; i++) {
            int nuevoX = actual.x + movX[direcciones[i]]; //Esto calcula la nueva posición horizontal (X)
...
int main() {
    ...
    // Crear el laberinto
    char** laberinto = new char*[filas]; //  es decir, una matriz dinámica de caracteres que representa el laberinto.
    for (int i = 0; i < filas; ++i) { //  repasar: laberinto apuntará a un arreglo de punteros, donde cada puntero apunta a un arreglo de caracteres.
